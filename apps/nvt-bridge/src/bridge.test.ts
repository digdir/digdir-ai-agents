...
async function harness(overrides: { maxParallel?: number } = {}): Promise<Harness> {
  const dir = await mkdtemp(path.join(tmpdir(), "nvt-bridge-b-"));
...
test("prompten bærer event-id, resultatkontrakten og oppgaveteksten avgrenset", async () => {
  ...
  assert.match(prompt, /SLUTT OPPGAVETEKST/);
});
